...
bool GenericDomTreeNodeBase::compare(
  ...
  SmallSet<BlockHandle, 4> OtherChildren;
  for (const GenericDomTreeNodeBase *I : *Other) {
    BlockHandle Nd = I->getBlock();
    OtherChildren.insert(Nd);
  }

  for (const GenericDomTreeNodeBase *I : *this) {
    BlockHandle N = I->getBlock();
...
bool GenericDominatorTreeBase::compare(
  ...
  for (const auto &DomTreeNode : DomTreeNodes) {
    BlockHandle BB = DomTreeNode.first;
...
bool GenericDominatorTreeBase::properlyDominatesBlock(BlockHandle A,
                                                      BlockHandle B) const {
  if (A == B)
    return false;
...
bool GenericDominatorTreeBase::dominatesBlock(BlockHandle A,
                                              BlockHandle B) const {
  if (A == B)
    return true;
...
BlockHandle
GenericDominatorTreeBase::findNearestCommonDominatorBlock(BlockHandle A,
                                                          BlockHandle B) const {
