...
public:
  GenericDomTreeNodeBase(BlockHandle BB, GenericDomTreeNodeBase *idom)
  ...
  BlockHandle getBlock() const { return TheBB; }
...
template <class NodeT> class DomTreeNodeBase : public GenericDomTreeNodeBase {
  using Wrapper = HandleWrapper<BlockHandle, NodeT *>;
...
public:
  DomTreeNodeBase(NodeT *BB, DomTreeNodeBase *IDom)
      : GenericDomTreeNodeBase(Wrapper::wrapRef(BB), IDom) {}
  ...
  NodeT *getBlock() const { return Wrapper::unwrapRef(TheBB); }
...
class GenericDominatorTreeBase {
protected:
  DenseMap<BlockHandle, std::unique_ptr<GenericDomTreeNodeBase>> DomTreeNodes;
...
public:
  ...
  GenericDomTreeNodeBase *getNode(BlockHandle BB) const {
  ...
  /// See getNode.
  GenericDomTreeNodeBase *operator[](BlockHandle BB) const {
    return getNode(BB);
  }
  ...
  bool properlyDominatesBlock(BlockHandle A, BlockHandle B) const;

  bool dominates(const GenericDomTreeNodeBase *A,
                 const GenericDomTreeNodeBase *B) const;
  bool dominatesBlock(BlockHandle A, BlockHandle B) const;

  const GenericDomTreeNodeBase *
  findNearestCommonDominator(const GenericDomTreeNodeBase *A,
                             const GenericDomTreeNodeBase *B) const;
  BlockHandle findNearestCommonDominatorBlock(BlockHandle A,
                                              BlockHandle B) const;
...
public:
  ...
  TreeNode *setNewRoot(NodeT *BB) {
    ...
    if (Roots.empty()) {
      ...
      auto &OldNode = DomTreeNodes[Wrapper::wrapRef(OldRoot)];
  ...
  void eraseNode(NodeT *BB) {
    ...
    DomTreeNodes.erase(Wrapper::wrapRef(BB));
