; RUN: opt < %s -cycleinfo -analyze -enable-new-pm=0 | FileCheck %s -check-prefix=CHECK
; RUN: opt < %s -disable-output -passes='print<cycleinfo>' 2>&1 | FileCheck %s -check-prefix=CHECK
