...
; Autogenerated checks look different in the presence of tokens. It's cleaner to check for a fragment of the call instructions.
; RUN: opt < %s -passes=loop-unroll -unroll-runtime -unroll-allow-partial -S | FileCheck %s
; RUN: opt < %s -passes='add-convergence-control,loop-unroll' -unroll-runtime -unroll-allow-partial -S | FileCheck %s
